...
// getGobRegisters generates the gob.Register calls for the types in the config.
...
// createGeneratorConfig creates a generator config from the init config.
func createGeneratorConfig(config InitConfig) Config {
	// Struct and member names
	lowercaseServiceStructName := strings.ToLower(
		config.ServiceStructName[:1],
	) + config.ServiceStructName[1:]
	lowercaseClientStructName := strings.ToLower(
		config.ClientStructName[:1],
	) + config.ClientStructName[1:]
	...
	return Config{
		...
		serviceStructName:              config.ServiceStructName,
		clientStructName:               config.ClientStructName,
...
func GenerateForInterface(
	...
	generatorConfig := createGeneratorConfig(config)
