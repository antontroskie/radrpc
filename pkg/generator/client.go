...
func GenerateClientMethods(
	...
	getFunctionReturns := func(method *types.Func) *jen.Statement {
	...
	getFunctionParams := func(method *types.Func) *jen.Statement {
		params := jen.Empty()
	...
	getArgNames := func(method *types.Func) *jen.Statement {
		args := jen.Empty()
		for i := 0; i < method.Type().(*types.Signature).Params().Len(); i++ {
	...
	generateMethodBody := func(method *types.Func) *jen.Statement {
		body := jen.Empty()
		...
		body.Add(jen.Id("msg").Op(":=").Id("itf").Dot("RPCMessageReq").Block(
			...
			jen.Id("Type").Op(":").Id("itf.MethodRequest").Add(jen.Op(",")),
	...
	generateMethod := func(method *types.Func) *jen.Statement {
		return jen.Func().
			Params(jen.Id("s").Op("*").Id(generatorConfig.clientStructName)).
			Id(method.Name()).
			Params(getFunctionParams(method)).
			Params(getFunctionReturns(method)).
			Block(generateMethodBody(method))
	}
	...
	for i := 0; i < generatorConfig.initConfig.InterfaceType.NumMethods(); i++ {
		...
		stmt.Add(generateMethod(method))
		stmt.Add(jen.Op(";"))
	}
